...
public class UITileManager : MonoBehaviour
{
    ...
    {
        tilesTraveled++;
        if (IsInnerTile(tilesTraveled)) return GetInnerTilePosition(playerType, tilesTraveled);
        int startingTileNo = GetStartingTileNo(playerType);
        int tilesReached = CalculateTileNo(startingTileNo, tilesTraveled);
        return GetTilePosition(tilesReached);
    }
    ...
    private int GetStartingTileNo(PlayerType playerType) => Constants.Tiles.StartingTilesNo[GetPlayerIndex(playerType)];
    private int GetStartingInnerTileNo(PlayerType playerType) => CalculateTileNo(GetStartingTileNo(playerType), Constants.Tiles.InnerTileStarting);
    private bool IsInnerTile(int tilesTraveled) => Constants.Tiles.InnerTileStarting <= tilesTraveled && tilesTraveled <= Constants.Tiles.InnerTileEnding;
    private int CalculateTileNo(int startingTile, int tileTraveled) => (startingTile - 1 + tileTraveled) % Constants.Tiles.TotalTilesInOuterPath;
    ...
    private int GetPlayerIndex(PlayerType playerType)
    {
    ...
    private Vector2 GetPositionToTop(Vector2 startingPosition, int tiles) => startingPosition + Vector2.up * tileHeight;
    private Vector2 GetPositionToBottom(Vector2 startingPosition, int tiles) => startingPosition + Vector2.down * tileHeight;
    private Vector2 GetPositionToLeft(Vector2 startingPosition, int tiles) => startingPosition + Vector2.left * tileWidth;
    private Vector2 GetPositionToRight(Vector2 startingPosition, int tiles) => startingPosition + Vector2.right * tileWidth;

    private Vector2 GetInnerTilePosition(PlayerType playerType, int tilesTraveled)
    {
        int noOfInnerTilesReached = tilesTraveled - Constants.Tiles.InnerTileStarting;
        Vector2 startingInnerTilePosition = GetTilePosition(GetStartingInnerTileNo(playerType));
        switch (GetPlayerIndex(playerType))
        {
    ...
    private int GetRowNo(int tileIndex)
    {
    ...
    private int GetColNo(int tileIndex)
    {
        // col -6 => 13 - 11
        // col -5 => 14 , 10
        // col -4 => 15 , 9
    ...
    {
        int row = GetRowNo(tileIndex);
        int col = GetColNo(tileIndex);
